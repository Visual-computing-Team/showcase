new p5((p) => {
    window.localStorage.removeItem("imgChosen");
    ...
    p.preload = function () {
        // Load an image
        if (imgCookie != "")
            img = p.loadImage(imgCookie, () => { window.alert("Imagen cargada!") }, () => { window.alert("Error al cargar la imagen. Ingrese una nueva URL"); document.cookie = "photo" + "=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;"; window.location.reload() });
        else
    ...
    p.setup = function () {
        setInterval(checkCookieValue, 1000); // check the storage value every second
        // Create a canvas
